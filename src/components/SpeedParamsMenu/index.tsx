...
import styles from './SpeedParamsMenu.module.scss';
...
export const SpeedParamsMenu = React.forwardRef<HTMLUListElement, SpeedParamsMenuProps>((props, ref) => {
  ...
  return (
    <ul ref={ref} className={clsx(styles.root, isSpeedParamsOpen && styles.rootIsOpened)}>
      {paramsMenuItems.map((paramMenuItem, index) => (
        <li key={index} className={styles.listItem}>
          <button onClick={paramMenuItem.onClick} className={styles.button}>
            {paramMenuItem.name}
          </button>
        </li>
      ))}
